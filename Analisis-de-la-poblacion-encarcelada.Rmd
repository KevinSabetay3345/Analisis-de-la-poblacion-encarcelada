---
title: "Analisis de la poblacion encarcelada"
...
```{r}
df = read.csv("C:/Users/kevin/OneDrive/Escritorio/Labo de datos/sneep-2021.csv")
...
cross_validation = function(df, size_group) {
  ...
  while (i + size_group < nrow(df)) {
    ajus = lm(duracion_condena_total ~ delito1 + edad, df[-i:-i-size_group,])
  ...
  ajus = lm(duracion_condena_total ~ delito1 + edad, df[-i:-nrow(df),])
  for (j in i:nrow(df)) {
    tryCatch({
      predichos[j] = predict(ajus, df[j,])
    }, error = function(cond) {
...
## Modelo predictivo de toda la poblacion
Tarda un rato en generar las muestras. Tiene muchas observaciones.

```{r}
all_model = cross_validation(df2, 5100)
paste('MAE:',all_model$mae,'PMAE:',all_model$pmae,sep = " ")
```

## Modelo predictivo con la poblacion de mujeres

```{r}
dfMujeres = df[df$duracion_condena_total > 0 & df$delito1 != "" & df$genero == "Mujer",]

mujeres_model = cross_validation(dfMujeres, 175)
paste('MAE:',mujeres_model$mae,'PMAE:',mujeres_model$pmae,sep = " ")
```
